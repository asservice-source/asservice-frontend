...
export class SummaryDiedComponent extends BaseComponent implements OnInit {
  public title: string = 'แบบสรุปข้อมูลผู้เสียชีวิต';
  ...
  setDropdownListRounds(){
    ...
    this.api.api_SurveyHeaderList(this.surveyHeaderCode.DEATH, data =>{
  ...
  onReporting(blankForm: boolean = false){
    let $form: any;
    if(blankForm){
      $form = $('<form method="post" target="_blank" name="mfrm" action="'+ReportPath.DEATH_BLANK_FORM+'"></form>');
    }else{
      this.inputvalidate = new InputValidateInfo();
