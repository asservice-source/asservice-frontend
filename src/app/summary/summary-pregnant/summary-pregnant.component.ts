...
export class SummaryPregnantComponent extends BaseComponent implements OnInit {
  public title = 'แบบสรุปหญิงตั้งครรภ์และหลังคลอด';
  ...
  setDropdownListRounds(){
    ...
    this.api.api_SurveyHeaderList(this.surveyHeaderCode.PREGNANT, data =>{
  ...
  onReporting(blankForm: boolean = false){
    let $form: any;
    if(blankForm){
      $form = $('<form method="post" target="_blank" name="mfrm" action="'+ReportPath.PREGNANCY_BLANK_FORM+'"></form>');
    }else{
      this.inputvalidate = new InputValidateInfo();
